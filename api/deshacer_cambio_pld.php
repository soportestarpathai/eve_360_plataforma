...
try {
    ...
    if ($tabla === 'avisos_pld') {
        $id_cliente = (int)($valor_anterior['id_cliente'] ?? 0);
    } elseif ($tabla === 'operaciones_pld') {
        $id_cliente = (int)($valor_anterior['id_cliente'] ?? 0);
    }
    ...
    $allowed = ['avisos_pld', 'operaciones_pld'];
    if (!in_array($tabla, $allowed)) {
        http_response_code(400);
        echo json_encode(['status' => 'error', 'message' => 'Solo se puede deshacer en avisos_pld u operaciones_pld']);
        exit;
    }

    $pk = ($tabla === 'avisos_pld') ? 'id_aviso' : 'id_operacion';
    $cols = [];
    $vals = [];
    foreach ($valor_anterior as $k => $v) {
        if ($k === $pk) continue;
        if (!in_array($k, ['folio_sppld', 'fecha_presentacion', 'estatus', 'monto', 'fecha_operacion', 'tipo_operacion', 'id_status', 'id_aviso_generado'])) {
            continue;
        }
        $cols[] = "`$k` = ?";
        $vals[] = $v;

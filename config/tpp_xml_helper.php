<?php
/**
 * Helper para generar XML TPP (Tarjetas Prepagadas y Cupones) - Fracción II
 * Según instructivo F.II-2 Tarj Prep.
 * Estructura: archivo > informe > sujeto_obligado > aviso > persona_aviso > detalle_operaciones
 */

if (!function_exists('formatMontoTpp')) {
    function formatMontoTpp($val): string {
...
if (!function_exists('tppToUpper')) {
    function tppToUpper($val): string {
        $v = trim((string)$val);
        if ($v === '') return '';
...
if (!function_exists('tppSanitizeRef')) {
    function tppSanitizeRef($val, $allowEnie = true): string {
        $v = tppToUpper($val);
...
if (!function_exists('tppSanitizeDesc')) {
    function tppSanitizeDesc($val): string {
        $v = tppToUpper(trim((string)$val));
        $v = preg_replace('/[^A-Z\x{00D1}0-9 \-\.,\':\/$]/u', '', $v);
        return substr($v, 0, 3000);
    }
}

if (!function_exists('tppSanitizeClaveSO')) {
    function tppSanitizeClaveSO($val): string {
        $v = tppToUpper(trim((string)$val));
        $v = preg_replace('/[^A-Z\x{00D1}0-9]/u', '', $v);
...
if (!function_exists('tppValidarClaveSO')) {
    function tppValidarClaveSO($val): bool {
        $v = trim((string)$val);
        if ($v === '') return false;
        return (bool) preg_match('/^[A-Z\x{00D1}&]{3,4}\d{6}[A-Z0-9]{3}$/u', tppToUpper($v));
    }
}

if (!function_exists('generateTPPXml')) {
    function generateTPPXml(array $data): array {
        $NS  = 'http://www.uif.shcp.gob.mx/recepcion/tpp';
        ...
        $addEl = function(DOMDocument $dom, DOMElement $parent, string $ns, string $name, $value) use ($UPPER_FIELDS) {
            ...
            if ($name === 'mes_reportado') {
                ...
                $value = substr(preg_replace('/[^A-Z\x{00D1}&0-9]/u', '', tppToUpper($value)), 0, 12);
                ...
                $value = formatMontoTpp($value);
            } elseif ($name === 'cantidad') {
                $value = preg_replace('/[^0-9]/', '', $value);
                $value = ltrim($value, '0');
                $value = $value === '' ? '0' : substr($value, 0, 8);
            } elseif ($name === 'clave_sujeto_obligado') {
                $value = tppSanitizeClaveSO($value);
            } elseif ($name === 'rfc') {
                $value = substr(preg_replace('/[^A-Z\x{00D1}&0-9]/u', '', tppToUpper($value)), 0, 13);
            } elseif ($name === 'curp') {
                $value = substr(preg_replace('/[^A-Z0-9]/u', '', tppToUpper($value)), 0, 18);
            } elseif (in_array($name, ['pais', 'pais_nacionalidad', 'clave_pais'], true)) {
                $value = substr(preg_replace('/[^A-Z]/', '', tppToUpper($value)), 0, 2);
            } elseif ($name === 'correo_electronico') {
                $value = substr(preg_replace('/[^A-Z0-9\._\'\-@]/', '', tppToUpper($value)), 0, 60);
                ...
                $value = tppSanitizeRef($value, true);
            } elseif (in_array($name, ['descripcion_alerta', 'descripcion_modificacion'], true)) {
                $value = tppSanitizeDesc($value);
            } elseif ($name === 'identificador_fideicomiso') {
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \-_\.&,\#@\']/u', '', tppToUpper($value)), 0, 40);
                ...
                $value = preg_replace('/[^0-9]/', '', $value);
                $value = strlen($value) >= 1 ? substr($value, 0, 2) : '';
                ...
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \#\-\.\&,_@\'\(\)]/u', '', tppToUpper($value)), 0, 254);
            } elseif (in_array($name, ['nombre', 'apellido_paterno', 'apellido_materno'], true)) {
                $value = substr(preg_replace('/[^A-Z\x{00D1} ]/u', '', tppToUpper($value)), 0, 200);
            } elseif ($name === 'colonia') {
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \-\.,:\/\(\)]/u', '', tppToUpper($value)), 0, 50);
            } elseif (in_array($name, ['calle', 'estado_provincia', 'ciudad_poblacion'], true)) {
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \-\.,:\/]/u', '', tppToUpper($value)), 0, 100);
            } elseif ($name === 'numero_exterior') {
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \-\.,:\/]/u', '', tppToUpper($value)), 0, 56);
            } elseif ($name === 'numero_interior') {
                $value = substr(preg_replace('/[^A-Z\x{00D1}0-9 \-\.,:\/]/u', '', tppToUpper($value)), 0, 40);
            } elseif (in_array($name, $UPPER_FIELDS, true)) {
                $value = tppToUpper($value);
            }
        ...
        $writeDomicilio = function(DOMDocument $dom, DOMElement $parent, string $ns, array $dData) use ($addEl, $mkEl) {
            if (isset($dData['nacional']) && is_array($dData['nacional'])) {
                ...
                if ($cpExt !== '') {
                    $cpExt = substr(preg_replace('/[^A-Z\x{00D1}0-9]/u', '', tppToUpper($cpExt)), 0, 12);
                }
        ...
        $archivo->setAttributeNS($XSI, 'xsi:schemaLocation', $NS . ' tpp.xsd');
        ...
        foreach ($data['informe'] as $inf) {
            ...
            $addEl($dom, $soEl, $NS, 'clave_actividad', !empty($so['clave_actividad']) ? $so['clave_actividad'] : 'TPP');

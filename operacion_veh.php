...
require_once 'config/veh_catalogos.php';
...
if (!function_exists('userCanAccessVEH') || !userCanAccessVEH($pdo, $userId)) {
    header('Location: operaciones_pld.php?error=sin_permiso_veh');
    exit;
}

$page_title = 'Aviso VEH - Fraccion VIII';
$claveSO = '';
try {
    $row = [];
    if ($userId > 0) {
...
<div class="veh-wrapper">
  <div class="veh-page-header">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <h2><i class="fa-solid fa-car-side me-2"></i>Aviso VEH</h2>
        <p>Fraccion VIII - Comercializacion y distribucion de vehiculos</p>
      </div>
      <a class="btn btn-outline-light btn-sm" href="operaciones_pld.php"><i class="fa-solid fa-arrow-left me-1"></i> Volver</a>
    </div>
  </div>

  <div class="veh-progress" aria-label="Progreso VEH">
    ...
    <div class="veh-step active"><div class="veh-step-num">5</div><div>Detalle VEH</div></div>
  </div>

  <div class="alert alert-info">Umbral de identificacion: <code>3,210 UMA</code>. Umbral de aviso/acumulacion: <code>6,420 UMA</code>.</div>

<form id="formVEH" novalidate>
  <div class="veh-card" id="sec-cliente">
    ...
    <div class="veh-card-body">
      <div class="row g-3">
        ...
        <div class="col-xl-2 col-lg-2 col-md-4"><label class="form-label">Actividad</label><input class="form-control" value="VEH" readonly></div>
        <div class="col-xl-2 col-lg-3 col-md-4"><label class="form-label">Exento</label><select id="exento" class="form-select"><?= vehCatalogoOptions('exento', '0', null, false) ?></select></div>
  ...
  <div class="veh-card" id="sec-aviso">
    ...
    <div class="veh-card-body">
      <div class="row g-3">
        ...
        <div class="col-lg-2 col-md-3"><label class="form-label">Prioridad *</label><select id="prioridad" class="form-select" required><?= vehCatalogoOptions('prioridad', '1', null, false) ?></select></div>
        <div class="col-lg-3 col-md-6"><label class="form-label">Tipo alerta *</label><select id="tipo_alerta" class="form-select" required><?= vehCatalogoOptions('tipo_alerta', '100') ?></select></div>
        <div class="col-lg-4 col-md-12"><label class="form-label">Descripcion alerta</label><input id="descripcion_alerta" class="form-control text-uppercase" maxlength="3000" placeholder="Ej. OPERACION INUSUAL EN COMPRA DE VEHICULO"></div>
      </div>
    ...
    <div class="veh-card-body">
      ...
      <div id="pf_wrap" class="mt-3">
        <div class="row g-3">
          ...
          <div class="col-lg-3 col-md-4"><label class="form-label">Pais *</label><select id="pf_pais" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          <div class="col-lg-6 col-md-8"><label class="form-label">Actividad economica *</label><select id="pf_act" class="form-select"><?= vehCatalogoOptions('actividad_economica', '1000000') ?></select></div>
        </div>
      </div>
      ...
      <div id="pm_wrap" class="mt-3 veh-hidden">
        <div class="row g-3">
          ...
          <div class="col-lg-3 col-md-4"><label class="form-label">Pais *</label><select id="pm_pais" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          <div class="col-lg-6 col-md-8"><label class="form-label">Giro mercantil *</label><select id="pm_giro" class="form-select"><?= vehCatalogoOptions('giro_mercantil', '1000000') ?></select></div>
        </div>
      ...
      <div id="fi_wrap" class="mt-3 veh-hidden">
        <div class="row g-3">
          <div class="col-lg-6 col-md-8"><label class="form-label">Denominacion fideicomiso *</label><input id="fi_den" class="form-control text-uppercase" placeholder="FIDEICOMISO VEHICULAR"></div>
      ...
      <div id="dom_ext" class="veh-hidden">
        <div class="row g-3">
          <div class="col-lg-3 col-md-6"><label class="form-label">Pais *</label><select id="de_pais" class="form-select"><?= vehCatalogoOptions('pais', 'US') ?></select></div>
      ...
      <div class="row g-3 mt-1">
        <div class="col-md-3"><label class="form-label">Clave pais telefono</label><select id="tel_pais" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
      ...
      <div class="d-flex flex-wrap gap-2 align-items-center mt-3">
        ...
        <span class="veh-note">Puedes registrar varias personas en el mismo aviso VEH.</span>
      </div>
      ...
      <div id="db_wrap" class="veh-hidden">
        ...
        <div id="db_fisica" class="mt-2">
          <div class="row g-3">
            ...
            <div class="col-md-3"><label class="form-label">Pais *</label><select id="db_f_pais" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          </div>
        </div>
        <div id="db_moral" class="mt-2 veh-hidden">
          <div class="row g-3">
            <div class="col-md-6"><label class="form-label">Denominacion / Razon *</label><input id="db_m_den" class="form-control text-uppercase" placeholder="HOLDING VEHICULAR SA"></div>
            ...
            <div class="col-md-3"><label class="form-label">Pais *</label><select id="db_m_pais" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          </div>
        </div>
  ...
  <div class="veh-card" id="sec-detalle">
    <div class="veh-card-header" onclick="toggleVehCard(this)">
      ...
      <div><h5>Detalle de operacion VEH</h5><small>Datos de vehiculo y liquidacion</small></div>
      <i class="fa-solid fa-chevron-down veh-chevron"></i>
    </div>
    <div class="veh-card-body">
      <div class="row g-3">
        <div class="col-lg-3 col-md-4"><label class="form-label">Fecha operacion *</label><input id="op_fecha" type="date" class="form-control" value="<?= date('Y-m-d') ?>" max="<?= date('Y-m-d') ?>" placeholder="2026-03-24"></div>
        <div class="col-lg-3 col-md-4"><label class="form-label">CP operacion *</label><input id="op_cp" class="form-control" maxlength="5" placeholder="56140" inputmode="numeric"></div>
        <div class="col-lg-4 col-md-6"><label class="form-label">Tipo operacion *</label><select id="op_tipo" class="form-select"><?= vehCatalogoOptions('tipo_operacion', '801') ?></select></div>
      </div>

      <hr>
      <div class="row g-3"><div class="col-lg-3 col-md-4"><label class="form-label">Tipo vehiculo *</label><select id="veh_tipo" class="form-select"><?= vehCatalogoOptions('tipo_vehiculo', 'terrestre', null, false) ?></select></div></div>

      <div id="veh_terrestre" class="mt-3">
        <div class="row g-3">
          <div class="col-lg-3 col-md-4"><label class="form-label">Marca / fabricante *</label><input id="vt_marca" class="form-control text-uppercase" maxlength="40" placeholder="NISSAN"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Modelo *</label><input id="vt_modelo" class="form-control text-uppercase" maxlength="40" placeholder="VERSA ADVANCE"></div>
          <div class="col-lg-2 col-md-4"><label class="form-label">Anio *</label><input id="vt_anio" class="form-control" maxlength="4" placeholder="2024" inputmode="numeric"></div>
          <div class="col-lg-4 col-md-6"><label class="form-label">VIN</label><input id="vt_vin" class="form-control text-uppercase" maxlength="17" placeholder="1HGCM82633A123456"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">REPUVE</label><input id="vt_repuve" class="form-control text-uppercase" maxlength="8" placeholder="ABC12345"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Placas</label><input id="vt_placas" class="form-control text-uppercase" maxlength="12" placeholder="ABC123A"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Nivel blindaje *</label><select id="vt_blindaje" class="form-select"><?= vehCatalogoOptions('tipo_blindaje', '1') ?></select></div>
        </div>
      </div>

      <div id="veh_maritimo" class="mt-3 veh-hidden">
        <div class="row g-3">
          <div class="col-lg-3 col-md-4"><label class="form-label">Marca / fabricante *</label><input id="vm_marca" class="form-control text-uppercase" maxlength="40" placeholder="YAMAHA"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Modelo *</label><input id="vm_modelo" class="form-control text-uppercase" maxlength="40" placeholder="242 LIMITED S"></div>
          <div class="col-lg-2 col-md-4"><label class="form-label">Anio *</label><input id="vm_anio" class="form-control" maxlength="4" placeholder="2023" inputmode="numeric"></div>
          <div class="col-lg-4 col-md-6"><label class="form-label">Numero de serie *</label><input id="vm_serie" class="form-control text-uppercase" maxlength="20" placeholder="MAR1234567890ABC"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Bandera</label><select id="vm_bandera" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Matricula</label><input id="vm_matricula" class="form-control text-uppercase" maxlength="12" placeholder="MX-1234"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Nivel blindaje *</label><select id="vm_blindaje" class="form-select"><?= vehCatalogoOptions('tipo_blindaje', '1') ?></select></div>
        </div>
      </div>

      <div id="veh_aereo" class="mt-3 veh-hidden">
        <div class="row g-3">
          <div class="col-lg-3 col-md-4"><label class="form-label">Marca / fabricante *</label><input id="va_marca" class="form-control text-uppercase" maxlength="40" placeholder="CESSNA"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Modelo *</label><input id="va_modelo" class="form-control text-uppercase" maxlength="40" placeholder="172 SKYHAWK"></div>
          <div class="col-lg-2 col-md-4"><label class="form-label">Anio *</label><input id="va_anio" class="form-control" maxlength="4" placeholder="2022" inputmode="numeric"></div>
          <div class="col-lg-4 col-md-6"><label class="form-label">Numero de serie *</label><input id="va_serie" class="form-control text-uppercase" maxlength="20" placeholder="AER1234567890XYZ"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Bandera</label><select id="va_bandera" class="form-select"><?= vehCatalogoOptions('pais', 'MX') ?></select></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Matricula</label><input id="va_matricula" class="form-control text-uppercase" maxlength="12" placeholder="XA-ABC"></div>
          <div class="col-lg-3 col-md-4"><label class="form-label">Nivel blindaje *</label><select id="va_blindaje" class="form-select"><?= vehCatalogoOptions('tipo_blindaje', '1') ?></select></div>
        </div>
      </div>

      <hr>
      <h6 class="mb-2">Liquidacion</h6>
      <div class="row g-3">
        ...
        <div class="col-lg-2 col-md-4"><label class="form-label">Forma pago *</label><select id="liq_forma_pago" class="form-select"><?= vehCatalogoOptions('forma_pago', '1') ?></select></div>
        <div class="col-lg-2 col-md-4"><label class="form-label">Instrumento *</label><select id="liq_instrumento" class="form-select"><?= vehCatalogoOptions('instrumento_monetario') ?></select></div>
        <div class="col-lg-2 col-md-4"><label class="form-label">Moneda *</label><select id="liq_moneda" class="form-select"><?= vehCatalogoOptions('moneda', '1') ?></select></div>
  ...
  <div class="veh-submit-bar mb-4">
    ...
    <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk me-1"></i> Registrar aviso VEH</button>
  </div>
</form>
</div>
</div>
...
function setupMasks() {
  ...
  bindDigits('vt_anio', 4);
  bindDigits('vm_anio', 4);
  bindDigits('va_anio', 4);
  ...
  ['pf_nombre','pf_ap','pf_am','pm_den','pm_rn','pm_rap','pm_ram','fi_den','fi_an','fi_aap','fi_aam','dn_col','dn_calle','dn_ne','dn_ni','de_est','de_cd','de_col','de_calle','de_ne','de_ni','db_f_nom','db_f_ap','db_f_am','db_m_den','db_t_den','db_t_id','descripcion_alerta','descripcion_modificacion','vt_marca','vt_modelo','vt_vin','vt_repuve','vt_placas','vm_marca','vm_modelo','vm_serie','vm_matricula','va_marca','va_modelo','va_serie','va_matricula','de_cp','tel_mail']
...
function tgVehTipo() {
  const t = v('veh_tipo');
  q('veh_terrestre').classList.toggle('veh-hidden', t !== 'terrestre');
  q('veh_maritimo').classList.toggle('veh-hidden', t !== 'maritimo');
  q('veh_aereo').classList.toggle('veh-hidden', t !== 'aereo');
}
...
function buildTipoVehiculo() {
  const t = v('veh_tipo');
  if (t === 'terrestre') return { datos_vehiculo_terrestre: { marca_fabricante: up(v('vt_marca')), modelo: up(v('vt_modelo')), anio: v('vt_anio'), vin: up(v('vt_vin')), repuve: up(v('vt_repuve')), placas: up(v('vt_placas')), nivel_blindaje: v('vt_blindaje') } };
  if (t === 'maritimo') return { datos_vehiculo_maritimo: { marca_fabricante: up(v('vm_marca')), modelo: up(v('vm_modelo')), anio: v('vm_anio'), numero_serie: up(v('vm_serie')), bandera: up(v('vm_bandera')), matricula: up(v('vm_matricula')), nivel_blindaje: v('vm_blindaje') } };
  return { datos_vehiculo_aereo: { marca_fabricante: up(v('va_marca')), modelo: up(v('va_modelo')), anio: v('va_anio'), numero_serie: up(v('va_serie')), bandera: up(v('va_bandera')), matricula: up(v('va_matricula')), nivel_blindaje: v('va_blindaje') } };
}
function validateVehiculo() {
  const t = v('veh_tipo');
  if (!['terrestre','maritimo','aereo'].includes(t)) throw new Error('Tipo de vehiculo invalido.');
  if (t === 'terrestre') {
    if (!v('vt_marca') || !v('vt_modelo') || !/^\d{4}$/.test(v('vt_anio'))) throw new Error('Vehiculo terrestre requiere marca, modelo y anio.');
    if (!v('vt_vin') && !v('vt_repuve') && !v('vt_placas')) throw new Error('Vehiculo terrestre requiere al menos VIN, REPUVE o placas.');
    if (!/^[1-9]$/.test(v('vt_blindaje'))) throw new Error('Nivel de blindaje terrestre invalido.');
    return;
  }
  if (t === 'maritimo') {
    if (!v('vm_marca') || !v('vm_modelo') || !/^\d{4}$/.test(v('vm_anio')) || !v('vm_serie')) throw new Error('Vehiculo maritimo requiere marca, modelo, anio y numero de serie.');
    if (!/^[1-9]$/.test(v('vm_blindaje'))) throw new Error('Nivel de blindaje maritimo invalido.');
    return;
  }
  if (!v('va_marca') || !v('va_modelo') || !/^\d{4}$/.test(v('va_anio')) || !v('va_serie')) throw new Error('Vehiculo aereo requiere marca, modelo, anio y numero de serie.');
  if (!/^[1-9]$/.test(v('va_blindaje'))) throw new Error('Nivel de blindaje aereo invalido.');
}
...
function validateLiqEditor() {
  ...
  const forma = v('liq_forma_pago');
  const instrumento = v('liq_instrumento');
  const instrumentoObligatorio = ['1', '2', '5'].includes(forma); // Contado, Diferido o Permuta
  if (instrumentoObligatorio && !/^\d{1,2}$/.test(instrumento)) throw new Error('Instrumento monetario invalido.');
  if (!instrumentoObligatorio && instrumento !== '' && !/^\d{1,2}$/.test(instrumento)) throw new Error('Instrumento monetario invalido.');
  ...
  if (forma === '5' && !['16','99'].includes(instrumento)) throw new Error('Cuando forma de pago es Permuta, instrumento debe ser 16 u 99.');
  if (['13','14'].includes(instrumento) && !(Number(v('liq_moneda')) >= 159 && Number(v('liq_moneda')) <= 179)) throw new Error('Para instrumento 13/14 la moneda debe estar entre 159 y 179.');
  if (instrumento !== '' && !['13','14'].includes(instrumento) && Number(v('liq_moneda')) >= 159 && Number(v('liq_moneda')) <= 179) throw new Error('Monedas 159-179 solo aplican a instrumento 13/14.');
}
...
function operacionObj() { return { fecha_operacion: d8(v('op_fecha')), codigo_postal: v('op_cp'), tipo_operacion: v('op_tipo'), tipo_vehiculo: buildTipoVehiculo(), datos_liquidacion: getLiquidacionesFinal() }; }

function payload() {
  const personasFinal = getPersonasAvisoFinal();
  const sujetoObligado = { clave_entidad_colegiada: up(v('clave_entidad_colegiada')), clave_sujeto_obligado: up(v('clave_sujeto_obligado')), clave_actividad: 'VEH', exento: v('exento') };
...
function validateGeneral() {
  ...
  if (v('tipo_alerta') === '9999' && !v('descripcion_alerta')) throw new Error('Capture descripcion de alerta para 9999.');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  q('veh_tipo').addEventListener('change', tgVehTipo);
  ...
  tgPersona(); tgDom(); tgMod(); tgDb(); tgDbTipo(); tgVehTipo();
  setPersonaActionButton(); setLiqActionButton(); renderPersonasList(); renderLiqList();
  q('liq_fecha_pago').value = todayYmd(); q('op_fecha').value = todayYmd();

  q('formVEH').addEventListener('submit', async ev => {
    ev.preventDefault();
    try {
      validateGeneral();
      const p = payload();
      Swal.fire({ title: 'Registrando...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
      const r = await fetch('api/registrar_aviso_veh.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) });
      const j = await r.json();
      if (!r.ok || j.status !== 'success') throw new Error(j.message || 'No fue posible registrar aviso VEH.');
      await Swal.fire({ icon: 'success', title: 'Aviso VEH registrado', text: `Operacion #${j.id_operacion}` });

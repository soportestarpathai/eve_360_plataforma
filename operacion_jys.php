...
require_once 'config/jys_catalogos.php';
...
if (!function_exists('userCanAccessJYS') || !userCanAccessJYS($pdo, $userId)) {
    header('Location: operaciones_pld.php?error=sin_permiso_jys');
    exit;
}

$page_title = 'Aviso JYS - Fraccion I';
$claveSO = '';
try {
    $row = [];
    if ($userId > 0) {
...
<div class="don-wrapper">
  <div class="don-page-header">
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
      <div>
        <h2><i class="fa-solid fa-dice me-2"></i>Aviso JYS</h2>
        <p>Fracción I - Juegos con apuesta, concursos o sorteos</p>
      </div>
      <a class="btn btn-outline-light btn-sm" href="operaciones_pld.php"><i class="fa-solid fa-arrow-left me-1"></i> Volver</a>
    </div>
  </div>

  <div class="don-progress" aria-label="Progreso JYS">
    ...
    <div class="don-step active"><div class="don-step-num">5</div><div>Detalle JYS</div></div>
  </div>

  <div class="alert alert-info">Umbral de identificación: <code>325 UMA</code>. Umbral de aviso/acumulación: <code>645 UMA</code>.</div>

<form id="formJYS" novalidate>
  <div class="don-card" id="sec-cliente">
    ...
    <div class="row g-3">
      ...
      <div class="col-xl-2 col-lg-2 col-md-4"><label class="form-label">Actividad</label><input class="form-control" value="JYS" readonly></div>
      <div class="col-xl-2 col-lg-3 col-md-4"><label class="form-label">Exento</label><select id="exento" class="form-select"><?= jysCatalogoOptions('exento', '0', null, false) ?></select></div>
  ...
  <div class="don-card" id="sec-aviso">
    ...
    <div class="row g-3">
      ...
      <div class="col-lg-2 col-md-3"><label class="form-label">Prioridad *</label><select id="prioridad" class="form-select" required><?= jysCatalogoOptions('prioridad', '1', null, false) ?></select></div>
      <div class="col-lg-3 col-md-6"><label class="form-label">Tipo alerta *</label><select id="tipo_alerta" class="form-select" required><?= jysCatalogoOptions('tipo_alerta', '100') ?></select></div>
      <div class="col-lg-4 col-md-12"><label class="form-label">Descripcion alerta</label><input id="descripcion_alerta" class="form-control text-uppercase" maxlength="3000" placeholder="Ej. MOVIMIENTOS INUSUALES DEL CLIENTE"></div>
    </div>
    ...
    <div id="modif_wrap" class="row g-3 mt-1 don-hidden">
      ...
      <div class="col-lg-8 col-md-12"><label class="form-label">Descripcion modificacion *</label><input id="descripcion_modificacion" class="form-control text-uppercase" maxlength="3000" placeholder="Ej. CORRECCION DE DATOS DEL CLIENTE"></div>
    </div>
    </div>
  </div>
  ...
  <div class="don-card" id="sec-persona">
    ...
    <div id="pf_wrap" class="mt-3">
      <div class="row g-3">
        ...
        <div class="col-lg-3 col-md-4"><label class="form-label">Pais *</label><select id="pf_pais" class="form-select"><?= jysCatalogoOptions('pais', 'MX') ?></select></div>
        <div class="col-lg-6 col-md-8"><label class="form-label">Actividad economica *</label><select id="pf_act" class="form-select"><?= jysCatalogoOptions('actividad_economica', '1000000') ?></select></div>
      </div>
    </div>

    <div id="pm_wrap" class="mt-3 don-hidden">
      <div class="row g-3">
        <div class="col-lg-6 col-md-8"><label class="form-label">Denominacion / Razon *</label><input id="pm_den" class="form-control text-uppercase" placeholder="COMERCIALIZADORA JUEGOS DEL CENTRO SA DE CV"></div>
        ...
        <div class="col-lg-3 col-md-4"><label class="form-label">Pais *</label><select id="pm_pais" class="form-select"><?= jysCatalogoOptions('pais', 'MX') ?></select></div>
        <div class="col-lg-6 col-md-8"><label class="form-label">Giro mercantil *</label><select id="pm_giro" class="form-select"><?= jysCatalogoOptions('giro_mercantil', '1000000') ?></select></div>
      </div>
    ...
    <div id="dom_ext" class="don-hidden">
      <div class="row g-3">
        <div class="col-lg-3 col-md-6"><label class="form-label">Pais *</label><select id="de_pais" class="form-select"><?= jysCatalogoOptions('pais', 'US') ?></select></div>
    ...
    <div class="row g-3 mt-1">
      <div class="col-md-3"><label class="form-label">Clave pais telefono</label><select id="tel_pais" class="form-select"><?= jysCatalogoOptions('pais', 'MX') ?></select></div>
      ...
      <div class="col-md-6"><label class="form-label">Correo electronico</label><input id="tel_mail" class="form-control text-uppercase" maxlength="60" placeholder="CONTACTO@EMPRESA.COM"></div>
    </div>
    <div class="d-flex flex-wrap gap-2 align-items-center mt-3">
      ...
      <span class="don-note">Puedes registrar varias personas en el mismo aviso JYS.</span>
    </div>
  ...
  <div class="don-card" id="sec-beneficiario">
    <div class="don-card-header" onclick="toggleDonCard(this)">
      ...
      <div><h5>DueÃ±o beneficiario (opcional)</h5><small>InformaciÃ³n de beneficiario final del aviso</small></div>
      <i class="fa-solid fa-chevron-down don-chevron"></i>
    </div>
    <div class="don-card-body">
    ...
    <div id="db_wrap" class="don-hidden">
      ...
      <div id="db_fisica" class="mt-2">
        <div class="row g-3">
          ...
          <div class="col-md-3"><label class="form-label">Pais</label><select id="db_f_pais" class="form-select"><?= jysCatalogoOptions('pais', 'MX') ?></select></div>
        </div>
      </div>
      <div id="db_moral" class="mt-2 don-hidden">
        <div class="row g-3">
          ...
          <div class="col-md-3"><label class="form-label">Pais</label><select id="db_m_pais" class="form-select"><?= jysCatalogoOptions('pais', 'MX') ?></select></div>
        </div>
      </div>
  ...
  <div class="don-card" id="sec-detalle">
    <div class="don-card-header" onclick="toggleDonCard(this)">
      ...
      <div><h5>Detalle de operación JYS</h5><small>Datos de operación, sucursal y liquidación</small></div>
      <i class="fa-solid fa-chevron-down don-chevron"></i>
    </div>
    <div class="don-card-body">
    <div class="row g-3">
      ...
      <div class="col-lg-3 col-md-4"><label class="form-label">Tipo sucursal *</label><select id="op_suc_tipo" class="form-select"><option value="propia">Sucursal propia</option><option value="operador">Sucursal operador</option></select></div>
      <div class="col-lg-4 col-md-6"><label class="form-label">Tipo operacion *</label><select id="op_tipo" class="form-select"><?= jysCatalogoOptions('tipo_operacion', '101') ?></select></div>
      <div class="col-lg-2 col-md-6"><label class="form-label">Linea negocio *</label><select id="op_linea" class="form-select"><?= jysCatalogoOptions('linea_negocio', '1') ?></select></div>
      <div class="col-lg-2 col-md-6"><label class="form-label">Medio operación *</label><select id="op_medio" class="form-select"><?= jysCatalogoOptions('medio_operacion', '1') ?></select></div>
    </div>
    <div id="suc_propia_wrap" class="row g-3 mt-1">
      <div class="col-lg-3 col-md-4"><label class="form-label">CP sucursal propia *</label><input id="op_cp_propia" class="form-control" maxlength="5" placeholder="01030" inputmode="numeric"></div>
    </div>
    <div id="suc_operador_wrap" class="row g-3 mt-1 don-hidden">
      <div class="col-lg-6 col-md-8"><label class="form-label">Nombre operador *</label><input id="op_operador_nombre" class="form-control text-uppercase" maxlength="254" placeholder="OPERADOR DEL SORTEO NACIONAL SA DE CV"></div>
      <div class="col-lg-3 col-md-4"><label class="form-label">CP sucursal operador *</label><input id="op_cp_operador" class="form-control" maxlength="5" placeholder="06000" inputmode="numeric"></div>
    </div>

    <div class="don-subcard mt-3">
      <div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-2">
        <h6 class="mb-0">Liquidaciones</h6>
        <span class="don-note">Puede agregar una o varias liquidaciones para la operación.</span>
      </div>
      <div class="row g-3 mt-1"><div class="col-lg-3 col-md-4"><label class="form-label">Clase liquidación *</label><select id="liq_clase" class="form-select"><option value="numerario">Numerario</option><option value="especie">Especie</option></select></div></div>
    <div id="liq_num_wrap" class="mt-2">
      <div class="row g-3">
        ...
        <div class="col-lg-3 col-md-4"><label class="form-label">Instrumento monetario *</label><select id="num_inst" class="form-select"><?= jysCatalogoOptions('instrumento_monetario') ?></select></div>
        <div class="col-lg-3 col-md-4"><label class="form-label">Moneda *</label><select id="num_moneda" class="form-select"><?= jysCatalogoOptions('moneda', '147') ?></select></div>
    ...
    <div id="liq_esp_wrap" class="mt-2 don-hidden">
      <div class="row g-3">
        <div class="col-md-3"><label class="form-label">Valor del bien *</label><input id="esp_valor_bien" type="number" class="form-control" step="0.01" min="0.01" value="0.00" placeholder="14500.00"></div>
        <div class="col-md-3"><label class="form-label">Moneda *</label><select id="esp_moneda" class="form-select"><?= jysCatalogoOptions('moneda', '147') ?></select></div>
        <div class="col-md-3"><label class="form-label">Bien liquidación *</label><select id="esp_bien" class="form-select"><?= jysCatalogoOptions('bien_liquidacion') ?></select></div>
      </div>
      <div id="inm_wrap" class="row g-3 mt-1 don-hidden">
        <div class="col-md-3"><label class="form-label">Tipo inmueble *</label><select id="inm_tipo" class="form-select"><?= jysCatalogoOptions('tipo_inmueble') ?></select></div>
        ...
        <div class="col-md-6"><label class="form-label">Folio real / antecedentes *</label><input id="inm_folio" class="form-control text-uppercase" maxlength="200" placeholder="FOLIO-REG-12345"></div>
      </div>
      <div id="otro_wrap" class="mt-2 don-hidden"><label class="form-label">Descripción del bien liquidación *</label><textarea id="otro_desc" class="form-control text-uppercase" rows="2" maxlength="3000" placeholder="EJ. PAQUETE DE FICHAS DE JUEGO"></textarea></div>
    </div>
      <div class="d-flex flex-wrap gap-2 align-items-center mt-3">
        <button type="button" id="btn_add_liq" class="btn btn-outline-success btn-sm"><i class="fa-solid fa-circle-plus me-1"></i>Agregar liquidación</button>
        <button type="button" id="btn_clear_liq" class="btn btn-outline-secondary btn-sm"><i class="fa-solid fa-eraser me-1"></i>Limpiar editor</button>
      </div>
  ...
  <div class="don-submit-bar mb-4">
    ...
    <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk me-1"></i> Registrar aviso JYS</button>
  </div>
</form>
</div>
</div>
...
function setLiquidacionActionButton() {
  const btn = q('btn_add_liq');
  if (!btn) return;
  if (liquidacionEditIndex === null) {
    btn.innerHTML = '<i class="fa-solid fa-circle-plus me-1"></i>Agregar liquidación';
    btn.classList.remove('btn-warning');
    btn.classList.add('btn-outline-success');
  } else {
    btn.innerHTML = '<i class="fa-solid fa-pen-to-square me-1"></i>Actualizar liquidación';
    btn.classList.remove('btn-outline-success');
    btn.classList.add('btn-warning');
  }
}

function setLiquidacionEditMode(index = null) {
  liquidacionEditIndex = Number.isInteger(index) && index >= 0 ? index : null;
  setLiquidacionActionButton();
}
...
function setupInputMasks() {
  ...
  bindInputMask('op_cp_propia', digits(5));
  bindInputMask('op_cp_operador', digits(5));
  bindInputMask('dn_cp', digits(5));
  bindInputMask('inm_cp', digits(5));
  bindInputMask('tel_num', digits(12));

  bindInputMask('de_cp', upperAllowed(12, /[^A-Z0-9Ã‘]/g));
  bindInputMask('folio_modificacion', upperAllowed(14, /[^0-9-]/g));

  bindInputMask('clave_sujeto_obligado', upperAllowed(13, /[^A-Z0-9Ã‘&]/g));
  bindInputMask('referencia_aviso', upperAllowed(14, /[^A-Z0-9Ã‘]/g));
  bindInputMask('clave_entidad_colegiada', upperAllowed(12, /[^A-Z0-9Ã‘&]/g));

  ['pf_rfc', 'pm_rrfc', 'fi_arfc', 'db_f_rfc'].forEach(id => bindInputMask(id, upperAllowed(13, /[^A-Z0-9Ã‘&]/g)));
  ['pm_rfc', 'fi_rfc', 'db_m_rfc', 'db_t_rfc'].forEach(id => bindInputMask(id, upperAllowed(12, /[^A-Z0-9Ã‘&]/g)));
  ...
  ['pf_nombre', 'pf_ap', 'pf_am', 'pm_den', 'pm_rn', 'pm_rap', 'pm_ram', 'fi_den', 'fi_an', 'fi_aap', 'fi_aam',
   ...
   'db_f_nom', 'db_f_ap', 'db_f_am', 'db_m_den', 'db_t_den', 'db_t_id', 'inm_folio', 'op_operador_nombre']
...
function tgSucursal() {
  const t = v('op_suc_tipo');
  q('suc_propia_wrap').classList.toggle('don-hidden', t !== 'propia');
  q('suc_operador_wrap').classList.toggle('don-hidden', t !== 'operador');
}
function tgMod() { q('modif_wrap').classList.toggle('don-hidden', !q('es_modificatorio').checked); }
function tgLiqClase() {
  const t = v('liq_clase');
  q('liq_num_wrap').classList.toggle('don-hidden', t !== 'numerario');
  q('liq_esp_wrap').classList.toggle('don-hidden', t !== 'especie');
}
function tgBien() {
  const b = v('esp_bien');
...
function domicilioEditorHasData() {
  const ids = ['dn_col', 'dn_calle', 'dn_ne', 'dn_ni', 'dn_cp', 'de_est', 'de_cd', 'de_col', 'de_calle', 'de_ne', 'de_ni', 'de_cp'];
  return ids.some(id => v(id) !== '');
}

function buildPersonaAvisoFromEditor() {
  const t = v('persona_tipo');
  const pa = { tipo_persona: personaObjFromEditor() };
  const domHasData = domicilioEditorHasData();
  if (t !== 'fideicomiso' || domHasData) {
    pa.tipo_domicilio = domObjFromEditor();
  }
  const telNum = v('tel_num');
  const telMail = up(v('tel_mail'));
  if (t !== 'fideicomiso' || telNum || telMail) {
...
function validatePersonaEditor() {
  const tipoPersona = v('persona_tipo');

  if (v('persona_tipo') === 'fisica') {
    if (!v('pf_nombre') || !v('pf_ap') || !v('pf_am')) throw new Error('Persona fÃ­sica incompleta.');
    if (!v('pf_pais') || !v('pf_act')) throw new Error('Persona fÃ­sica requiere paÃ­s y actividad econÃ³mica.');
    ...
    if (!v('fi_den')) throw new Error('Fideicomiso requiere denominaciÃ³n.');
    if (!v('fi_an') || !v('fi_aap') || !v('fi_aam')) throw new Error('Capture apoderado/delegado del fideicomiso.');
  }

  if (tipoPersona !== 'fideicomiso' || domicilioEditorHasData()) {
    if (v('domicilio_tipo') === 'nacional') {
      if (!v('dn_col') || !v('dn_calle') || !v('dn_ne') || !isCP(v('dn_cp'))) throw new Error('Domicilio nacional incompleto o CP invÃ¡lido.');
    } else {
      if (!v('de_pais') || !v('de_est') || !v('de_cd') || !v('de_col') || !v('de_calle') || !v('de_ne')) throw new Error('Domicilio extranjero incompleto.');
      if (!/^[A-Z0-9]{4,12}$/.test(up(v('de_cp')))) throw new Error('CÃ³digo postal extranjero invÃ¡lido.');
    }
  }

  if (tipoPersona !== 'fideicomiso') {
    if (!v('tel_pais') || !isPhone(v('tel_num'))) throw new Error('Teléfono requerido (10 o 12 dígitos) y clave de país válida.');
  } else if (v('tel_num') || v('tel_mail')) {
...
function resumenPersona(pa, index) {
  ...
  if (tp.persona_fisica) {
    const p = tp.persona_fisica;
    return `#${index + 1} FÃ­sica: ${p.nombre} ${p.apellido_paterno} ${p.apellido_materno}`.trim();
  }
  if (tp.persona_moral) {
...
function renderPersonasList() {
  ...
  if (personasAviso.length === 0) {
    cont.innerHTML = '<div class="don-note">No hay personas agregadas todavÃ­a. Puedes enviar solo la del editor actual o agregar varias.</div>';
    return;
  }
...
function buildLiquidacionFromEditor() {
  if (v('liq_clase') === 'numerario') {
    return {
      liquidacion_numerario: {
        fecha_pago: d8(v('num_fp')),
        instrumento_monetario: v('num_inst'),
        moneda: v('num_moneda'),
        monto_operacion: v('num_monto')
      }
    };
  }
  const especie = {
    valor_bien: v('esp_valor_bien'),
    moneda: v('esp_moneda'),
    bien_liquidacion: v('esp_bien')
  };
  if (v('esp_bien') === '1') {
    especie.datos_bien_liquidacion = {
      datos_inmueble: {
        tipo_inmueble: v('inm_tipo'),
        codigo_postal: v('inm_cp'),
        folio_real: up(v('inm_folio'))
      }
    };
  } else if (v('esp_bien') === '99') {
    especie.datos_bien_liquidacion = {
      datos_otro: {
        descripcion_bien_liquidacion: up(v('otro_desc'))
      }
    };
  }
  return { liquidacion_especie: especie };
}

function validateLiquidacionEditor() {
  if (v('liq_clase') === 'numerario') {
  ...
  if (!v('esp_moneda') || !v('esp_bien') || Number(v('esp_valor_bien')) <= 0) throw new Error('Especie requiere moneda, bien y valor > 0.');
  if (v('esp_bien') === '1' && (!v('inm_tipo') || !isCP(v('inm_cp')) || !v('inm_folio'))) throw new Error('Inmueble requiere tipo, CP y folio real.');
  if (v('esp_bien') === '99' && !v('otro_desc')) throw new Error('Capture descripción del bien liquidación.');
}

function clearLiquidacionEditor() {
  const ids = ['num_fp', 'num_monto', 'esp_valor_bien', 'inm_cp', 'inm_folio', 'otro_desc'];
  ids.forEach(id => { if (q(id)) q(id).value = ''; });
  q('num_fp').value = '<?= date('Y-m-d') ?>';
  q('liq_clase').value = 'numerario';
  if (q('num_inst').options.length > 0) q('num_inst').selectedIndex = 0;
  if (q('num_moneda').options.length > 0) q('num_moneda').value = '147';
  if (q('esp_moneda').options.length > 0) q('esp_moneda').value = '147';
  if (q('esp_bien').options.length > 0) q('esp_bien').value = '1';
  tgLiqClase();
  tgBien();
  setLiquidacionEditMode(null);
}

function loadLiquidacionToEditor(liq) {
  if (!liq || typeof liq !== 'object') return;
  clearLiquidacionEditor();
  if (liq.liquidacion_numerario) {
    const n = liq.liquidacion_numerario;
    q('liq_clase').value = 'numerario';
    q('num_fp').value = date8ToInput(n.fecha_pago);
    q('num_inst').value = n.instrumento_monetario || '';
    q('num_moneda').value = n.moneda || '';
    q('num_monto').value = n.monto_operacion || '';
  } else if (liq.liquidacion_especie) {
    const e = liq.liquidacion_especie;
    q('liq_clase').value = 'especie';
    q('esp_valor_bien').value = e.valor_bien || '';
    q('esp_moneda').value = e.moneda || '';
    q('esp_bien').value = e.bien_liquidacion || '';
    const db = e.datos_bien_liquidacion || {};
    if (db.datos_inmueble) {
      q('inm_tipo').value = db.datos_inmueble.tipo_inmueble || '';
      q('inm_cp').value = db.datos_inmueble.codigo_postal || '';
      q('inm_folio').value = db.datos_inmueble.folio_real || '';
    } else if (db.datos_otro) {
      q('otro_desc').value = db.datos_otro.descripcion_bien_liquidacion || '';
    }
  }
  tgLiqClase();
  tgBien();
...
function resumenLiquidacion(liq, index) {
  if (liq && liq.liquidacion_numerario) {
    const n = liq.liquidacion_numerario;
    return `#${index + 1} Numerario: $${n.monto_operacion} (${n.moneda})`;
  }
  if (liq && liq.liquidacion_especie) {
    const e = liq.liquidacion_especie;
    return `#${index + 1} Especie: $${e.valor_bien} (${e.moneda}) bien ${e.bien_liquidacion}`;
  }
  return `#${index + 1} Liquidación`;
}

function renderLiquidacionesList() {
  const cont = q('liq_list');
  if (!cont) return;
  if (liquidaciones.length === 0) {
    cont.innerHTML = '<div class="don-note">No hay liquidaciones agregadas todavía.</div>';
    return;
  }
  cont.innerHTML = liquidaciones.map((liq, i) => (
    `<div class="don-person-item d-flex justify-content-between align-items-center ${liquidacionEditIndex === i ? 'border border-warning' : ''}">
      <div>${resumenLiquidacion(liq, i)} ${liquidacionEditIndex === i ? '<span class="badge bg-warning text-dark ms-2">Editando</span>' : ''}</div>
      <div class="d-flex gap-1">
        <button type="button" class="btn btn-sm btn-outline-primary" data-edit-liq="${i}"><i class="fa-solid fa-pen me-1"></i>Editar</button>
        <button type="button" class="btn btn-sm btn-outline-danger" data-remove-liq="${i}"><i class="fa-solid fa-trash me-1"></i>Quitar</button>
      </div>
    </div>`
  )).join('');
}
...
function getLiquidacionesFinal() {
  if (liquidaciones.length > 0 && liquidacionEditIndex === null) {
    return [...liquidaciones];
  }
  validateLiquidacionEditor();
  const finalList = [...liquidaciones];
  const liq = buildLiquidacionFromEditor();
  if (liquidacionEditIndex !== null && finalList[liquidacionEditIndex]) {
    finalList[liquidacionEditIndex] = liq;
  } else {
    finalList.push(liq);
  }
  return finalList;
...
function operacionObj() {
  const op = {
    ...
    linea_negocio: v('op_linea'),
    medio_operacion: v('op_medio')
  };

  if (v('op_suc_tipo') === 'operador') {
    op.tipo_sucursal = {
      datos_sucursal_operador: {
        nombre_operador: up(v('op_operador_nombre')),
        codigo_postal: v('op_cp_operador')
      }
    };
  } else {
    op.tipo_sucursal = {
      datos_sucursal_propia: {
        codigo_postal: v('op_cp_propia')
      }
    };
  }
  op.datos_liquidacion = getLiquidacionesFinal();
  return op;
}
...
function validateGeneral() {
  ...
  if (!isMonth6(v('mes_reportado')) || v('mes_reportado') < '201309' || v('mes_reportado') > currYm) throw new Error('mes_reportado invÃ¡lido.');
  if (!/^[A-ZÃ‘&]{3,4}\d{6}[A-Z0-9]{3}$/.test(up(v('clave_sujeto_obligado')))) throw new Error('Clave sujeto obligado invÃ¡lida.');
  if (v('clave_entidad_colegiada') && !/^[A-ZÃ‘&]{3}\d{6}[A-Z0-9]{3}$/.test(up(v('clave_entidad_colegiada')))) throw new Error('Clave entidad colegiada invÃ¡lida.');
  if (!/^[A-ZÃ‘0-9]{1,14}$/.test(up(v('referencia_aviso')))) throw new Error('Referencia de aviso invÃ¡lida.');
  if (v('prioridad') === '2' && v('tipo_alerta') === '100') throw new Error('Prioridad 2 no permite alerta 100.');
  if (v('tipo_alerta') === '9999' && !v('descripcion_alerta')) throw new Error('Capture descripciÃ³n de alerta para 9999.');
  if (q('es_modificatorio').checked) {
    if (!/^[2-9]\d{3}-[1-9]\d{0,8}$/.test(up(v('folio_modificacion')))) throw new Error('Folio de modificaciÃ³n invÃ¡lido.');
    if (!v('descripcion_modificacion')) throw new Error('Capture descripciÃ³n de modificaciÃ³n.');
  }

  if (q('db_on').checked) {
    if (v('db_tipo') === 'fisica') {
      if (!v('db_f_nom') || !v('db_f_ap') || !v('db_f_am')) throw new Error('DueÃ±o beneficiario fÃ­sico incompleto.');
    } else if (v('db_tipo') === 'moral') {
      if (!v('db_m_den')) throw new Error('DueÃ±o beneficiario moral requiere denominaciÃ³n.');
    } else {
      if (!v('db_t_den') || !v('db_t_id')) throw new Error('DueÃ±o beneficiario fideicomiso requiere denominaciÃ³n e identificador.');
    }
  }

  if (!isDate8(d8(v('op_fecha'))) || !v('op_tipo') || !v('op_linea') || !v('op_medio')) throw new Error('Datos de operación inválidos.');
  if (!/^\d{1,2}$/.test(v('op_linea'))) throw new Error('Línea de negocio inválida (1-2 dígitos).');
  if (!/^\d{1}$/.test(v('op_medio'))) throw new Error('Medio de operación inválido (1 dígito).');
  if (v('op_suc_tipo') === 'operador') {
    if (!v('op_operador_nombre') || !isCP(v('op_cp_operador'))) throw new Error('Sucursal operador requiere nombre y código postal válido.');
  } else {
    if (!isCP(v('op_cp_propia'))) throw new Error('Sucursal propia requiere código postal válido.');
  }
  const liqFinal = getLiquidacionesFinal();
  if (!Array.isArray(liqFinal) || liqFinal.length === 0) throw new Error('Agregue al menos una liquidación.');
}
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  q('op_suc_tipo').addEventListener('change', tgSucursal);
  q('es_modificatorio').addEventListener('change', tgMod);
  q('liq_clase').addEventListener('change', tgLiqClase);
  ...
  q('btn_add_liq').addEventListener('click', () => {
    try {
      const isUpdate = liquidacionEditIndex !== null && !!liquidaciones[liquidacionEditIndex];
      validateLiquidacionEditor();
      if (isUpdate) {
        liquidaciones[liquidacionEditIndex] = buildLiquidacionFromEditor();
      } else {
        liquidaciones.push(buildLiquidacionFromEditor());
      }
      renderLiquidacionesList();
      clearLiquidacionEditor();
      Swal.fire({ icon: 'success', title: isUpdate ? 'Liquidación actualizada' : 'Liquidación agregada', timer: 1100, showConfirmButton: false });
    } catch (e) {
      Swal.fire({ icon: 'error', title: 'No fue posible agregar', text: e.message || 'Error inesperado' });
    }
  });

  q('btn_clear_liq').addEventListener('click', clearLiquidacionEditor);
  ...
  q('liq_list').addEventListener('click', ev => {
    ...
    if (btnEdit) {
      ...
      loadLiquidacionToEditor(liquidaciones[index]);
      setLiquidacionEditMode(index);
      renderLiquidacionesList();
      return;
    }
    ...
    if (liquidacionEditIndex !== null) {
      if (liquidacionEditIndex === index) {
        clearLiquidacionEditor();
      } else if (liquidacionEditIndex > index) {
        setLiquidacionEditMode(liquidacionEditIndex - 1);
      }
    }
    renderLiquidacionesList();
  });
  ...
  q('formJYS').addEventListener('submit', async ev => {
    ev.preventDefault();
    try {
      validateGeneral();
      const p = payload();
      Swal.fire({ title: 'Registrando...', allowOutsideClick: false, didOpen: () => Swal.showLoading() });
      const r = await fetch('api/registrar_aviso_jys.php', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(p) });
      const j = await r.json();
      if (!r.ok || j.status !== 'success') throw new Error(j.message || 'No fue posible registrar aviso JYS.');
      await Swal.fire({ icon: 'success', title: 'Aviso JYS registrado', text: `Operacion #${j.id_operacion}` });
